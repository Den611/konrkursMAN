...
def fix_db_safe():
    try:
        db.execute("ALTER TABLE feedback ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'Новий'")
    except:
        pass
...
class AdminApp(tk.Tk):
    ...
    def setup_ui(self):
        ...
        lbl_logo = tk.Label(self.sidebar, text="🔥 WordBot Admin", font=("Segoe UI", 14, "bold"))
        lbl_logo.pack(pady=25, padx=15, anchor="w")
        self.theme_widgets['labels'].append(lbl_logo)

        # Статус підключення
        lbl_status = tk.Label(self.sidebar, text=f"Джерело:\n{db.active_source}", font=("Segoe UI", 9),
        ...
        self.btn_nav_msg = self.create_nav_button(self.sidebar, "✉️ Відгуки та Розсилка", lambda: self.switch_tab(1))

        # Кнопка Бекапу
        self.btn_backup = self.create_nav_button(self.sidebar, "💾 Зробити Бекап", self.create_local_backup)
    ...
    def switch_tab(self, index):
        ...
        self.btn_backup.configure(bg=inact_bg, fg=inact_fg)  # Бекап ніколи не "активний" як вкладка
    ...
    def build_dashboard_tab(self):
        ...
        self.users_tree = ttk.Treeview(card_users, columns=("id", "name", "style", "hobby", "score", "active"),
        ...
        self.users_tree.heading("style", text="Психотип",
        ...
        self.users_tree.heading("hobby", text="Хобі",
        ...
        self.users_tree.heading("score", text="Рекорд",
        ...
        self.words_tree = ttk.Treeview(card_words, columns=("word", "trans", "lang", "interval"), show="headings")
        ...
        self.words_tree.heading("interval", text="Інтервал (SuperMemo)",
        ...
        self.create_action_button(frame_tools, "📥 Експорт", self.export_csv).pack(side=tk.RIGHT, padx=5)
        self.create_action_button(frame_tools, "📤 Імпорт", self.import_csv).pack(side=tk.RIGHT, padx=5)
    ...
    def build_messages_tab(self):
        ...
        for col, name in {"id": "ID", "user": "Користувач", "date": "Дата", "msg": "Відгук",
                          "status": "Статус"}.items():
        ...
        self.create_action_button(card_fb, "✍️ Відповісти", self.reply_feedback).pack(side=tk.RIGHT, padx=10, pady=5)
        self.create_action_button(card_fb, "✅ Прочитано", self.mark_feedback_read).pack(side=tk.RIGHT, padx=5, pady=5)
    ...
    def reply_feedback(self):
        ...
        if not sel: return messagebox.showwarning("Увага", "Оберіть відгук!")
        ...
        def send_reply():
            ...
            if res:
                uid = res[0]
                url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage"
                r = requests.post(url, json={"chat_id": uid, "text": f"💬 <b>Відповідь Адміністратора:</b>\n\n{msg}",
                                             "parse_mode": "HTML"})
                if r.status_code == 200:
                    db.execute("UPDATE feedback SET status='Відповіли' WHERE id=%s", (fb_id,))
                    self.update_feedback_table()
    ...
    def update_feedback_table(self):
        ...
        try:
            feedbacks = db.fetchall("SELECT id, username, date, message, status FROM feedback ORDER BY id DESC")
            c = self.colors
    ...
    def update_users_table(self):
        ...
        try:
            ...
            self.combo_users['values'] = ["Всі користувачі"] + [f"{u[0]} | @{u[1]}" for u in users]
            ...
            for u in users:
                ...
                if u[3]:
                    try:
                        if now - datetime.fromisoformat(str(u[3])) < timedelta(minutes=ACTIVE_THRESHOLD_MINUTES):
                            active = True
                    except:
                        pass
            ...
            for index, (active, u) in enumerate(rows):
                ...
                item = self.users_tree.insert("", tk.END,
                                              values=(u[0], f"@{u[1]}" if u[1] else "Без імені", u[5] or "Немає",
                                                      u[6] or "Немає", u[4] or 0, "Онлайн 🟢" if active else "Офлайн ⚪"),
                                              tags=tags)
    ...
    def load_words(self, uid, filter_lang="Усі мови"):
        ...
        try:
            if filter_lang and filter_lang != "Усі мови":
                ...
                words = db.fetchall("SELECT word, translation, language, interval FROM user_words WHERE user_id=%s",
                                    (uid,))
    ...
    def update_details(self, uid):
        ...
        try:
            data = db.fetchone("SELECT username, best_score FROM users WHERE user_id=%s", (uid,))
            if data:
    ...
    def export_csv(self):
        ...
        try:
            words = db.fetchall("SELECT word, translation, language FROM user_words WHERE user_id=%s",
    ...
    def create_local_backup(self):
        response = messagebox.askyesno("Бекап Бази Даних",
        ...
        try:
            ...
            cur.executemany("INSERT INTO user_words VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", words)

            # Перетягуємо feedback
            feedbacks = db.fetchall("SELECT id, user_id, username, message, date, status FROM feedback")
            cur.executemany("INSERT INTO feedback VALUES (?, ?, ?, ?, ?, ?)", feedbacks)

            local_db.commit()
            local_db.close()
            messagebox.showinfo("Успіх!", "Бекап успішно збережено у файл backup_words.db!")
