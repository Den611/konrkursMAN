...
class RotationManager:
    ...
    def rotate(self):
        ...
        if self.current_model_idx >= len(self.models):
            ...
            logger.info(
                f"🔑 [AI] Ліміти моделей вичерпано. Перемикаємо на ключ №{self.current_key_idx + 1}"
            )
        logger.info(
            f"🔄 [AI] Використовуємо модель: {self.models[self.current_model_idx]}"
        )
...
async def generate_content_safe(
    prompt: str, uid: int = 0, model_name: str = "gemma:2b"
) -> str:
    if not FORCE_GEMINI:
    ...
    if not rotation_manager.keys:
        return (
            ul(uid, "errors.ai_busy")
            if uid
            else "😅 ШІ зараз перевантажений! Спробуй пізніше."
        )

    attempts, max_attempts = (
        0,
        len(rotation_manager.keys) * len(rotation_manager.models) + 1,
    )
        ...
        try:
            ...
            async with _session.post(
                ...
                if response.status == 200:
                    ...
                    logger.error(
                        f"⚠️ [AI] Помилка {response.status} від {current_model}: {error_text}"
                    )
            ...
            logger.error(
                f"⚠️ [AI] Збій підключення до {current_model}: {type(e).__name__}"
            )
            rotation_manager.rotate()
            attempts += 1
    ...
    return (
        ul(uid, "errors.ai_busy")
        if uid
        else "😅 ШІ зараз обробляє багато запитів. Спробуй через хвилину!"
    )
...
async def generate_premium_content_safe(prompt: str, uid: int = 0) -> str:
        ...
        try:
            ...
            async with _session.post(
                ...
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    logger.error(
                        f"⚠️ [AI PREMIUM] Помилка: {response.status} - {error_text}"
                    )
                    if response.status in [429, 403, 404, 500, 503]:
                        rotation_manager.rotate()
                        attempts += 1
    ...
    return (
        ul(uid, "errors.ai_premium_busy")
        if uid
        else "😅 Розумна нейромережа зараз зайнята. Спробуй пізніше!"
    )
...
async def get_image_url(query, use_random=False):
    ...
    try:
        ...
        async with _session.get(
            "https://pixabay.com/api/", params=params, timeout=timeout
        ) as resp:
            if resp.status == 200:
                data = await resp.json()
                if data.get("hits"):
                    return (
                        random.choice(data["hits"])["webformatURL"]
                        if use_random
...
async def get_full_word_info(
    word: str,
    translation: str,
    lang: str,
    response_lang: str = "Ukrainian",
    uid: int = 0,
) -> dict:
    ...
    if result["association"] == translation:
        for line in text.splitlines():
            if (
                "асоціація" in line.lower()
                or "skojarzenie" in line.lower()
                or "association" in line.lower()
            ):
                if ":" in line:
...
# ДОДАНО параметр uid: int = 0
async def get_ai_explanation_text(
    content: str,
    language_of_word: str,
